...
import {
  ...
  EDIT_SELLER,
  FETCH_PRODUCT,
  FETCH_PRODUCTS, FETCH_PRODUCTS_BY_SELLER,
  FETCH_SELLER,
  FETCH_SELLERS,
  SIGN_IN,
  SIGN_OUT
...
export const fetchProductsBySellerId = id => async dispatch => {
  const response = await dataAccess.get(`/products/sellerId/${id}`);
  dispatch({
    type: FETCH_PRODUCTS_BY_SELLER,
...
export const createProduct = formValues => async (dispatch, getState) => {
  ...
  const response = await dataAccess.post("/products", {
    ...formValues,
    userId
  });
  dispatch({
